...
class candidateTable(CreateDB):
    ...
    def __init__(self):
        ...
        # create the dependency table for staff
    ...
    def sample_query(self):
        ...
        data = self.db.execute(
            "SELECT candidate_name,candidate_id,staff_id FROM candidate LIMIT 10")
        for row in data:
            logger.debug(row)
        return data
    ...
    def add_new_names(self):
        if json_df_dict != {} or candidate_df.empty == False:
            ...
            spartan_list2 = set(
                json_df_dict['sparta_day_df']['candidate_name'].tolist())
            ...
            logger.warning(
                f'New names from .json and academy_csv added to condidate_df\n{new_students}\n')
            for name in tqdm(new_students, unit='name', desc='Adding_new_candidates', position=0):
                self.candidate_df = self.candidate_df.append(
                    {'candidate_name': name}, ignore_index=True)
