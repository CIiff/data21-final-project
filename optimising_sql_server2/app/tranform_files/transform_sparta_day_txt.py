from optimising_sql_server2.app.load_files.get_files_from_s3 import getFiles, logger, pd, tqdm
...
class transformTxtFiles():
    ...
    def __init__(self):
        self.txt_files = getFiles(
            'data21-final-project-preassignment', 'Talent', '.txt')
    ...
    def make_dataframe(self):
        ...
        if self.txt_file_dict != {}:
            ...
            for sparta_day in tqdm(self.txt_file_dict, unit='txt_files', desc='Transforming_txt_files', position=0):
                ...
                text_lines_list2 = [line.replace(
                    "-  Psychometrics", ",Pychometrics") for line in text_lines_list]
                text_lines_list3 = [line.replace(
                    " - ", "-") for line in text_lines_list2]
                ...
                for spartan in flattened_list[3:-1]:
                    ...
                    spartan_phsycometrics = [scores.split(":")[1].strip().split(
                        "/") for scores in spartan[1].split(",")]
                    spartan_presentation = [scores.split(":")[1].strip().split(
                        "/") for scores in spartan[2].split(",")]
                    spartan_row = [spartan[0].strip().title()] + \
                        spartan_phsycometrics + spartan_presentation
                    # pprint(spartan_row)
                    candidate_name, presentation, presentation_max, psychometrics, psychometrics_max = spartan_row[
                        0], spartan_row[1][0], spartan_row[1][1], spartan_row[2][0], spartan_row[2][1]
                    ...
                    self.sparta_day_df = self.sparta_day_df.append(
                        sparta_day_details, ignore_index=True)
            self.sparta_day_df['date'] = self.sparta_day_df['date'].astype(
                'datetime64')
            ...
            logger.info('No new Sparta_Day txt files found')
            return pd.DataFrame()


sparta_day = transformTxtFiles()
txt_sparta_day_df = sparta_day.make_dataframe()
# print(sparta_day_df)
