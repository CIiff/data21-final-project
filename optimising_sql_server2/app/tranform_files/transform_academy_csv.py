...
class transformAcedamyCSV:
    ...
    def __init__(self):
        ...
        self.academy_csvs = getFiles(
            'data21-final-project-preassignment', 'Academy', '.csv')
            ...
            for key in tqdm(self.academy_csv_dfs_dict, unit='csv_files', desc='Tranforming_Academy_csv', position=0):
                ...
                for name in list(df.index.unique()):
                    ...
                    for wk in range(1, duration+1):
                        ...
                        self.weekly_performance_df = self.weekly_performance_df.append(
                            week_perfomance, ignore_index=True)
    ...
    def wk_performance_df(self):
        ...
        if df.empty == False:
            df['trainer'] = df['trainer'].str.replace(
                'Ely Kely', 'Elly Kelly', regex=True)
            ...
            for trainer_name in list(df['trainer'].unique()):
                ...
                if match != None:
                    logger.warning(
                        f'{trainer_name} is {match[1]}% similar to {match[0]}')
    ...
    def course_table_df(self):
        if self.academy_csv_dfs_dict != {}:
            for key in self.academy_csv_dfs_dict:
                ...
                self.course_df = self.course_df.append(
                    course_details, ignore_index=True)
