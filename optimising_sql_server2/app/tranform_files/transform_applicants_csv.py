...
class transformAppCSV:
    ...
    def __init__(self):
        ...
        self.talent_csvs = getFiles(
            'data21-final-project-preassignment', 'Talent', '.csv')
    ...
    def transform_dfs(self):
        if self.talent_csvs_list != []:
            for key in tqdm(self.talent_csv_df_dict.keys(), unit='Applicants_csv_files', desc='Tranforming Applicants_csv_files', position=0):
                ...
                self.talent_csv_df_dict[key]['phone_number'] = self.talent_csv_df_dict[key]['phone_number'].str.replace(
                    r'[^+\w]', '', regex=True)
                ...
                for row in self.talent_csv_df_dict[key].itertuples():
                    if type(row.phone_number) != float:
                        if row.phone_number[1:13].isdigit() == False or row.phone_number[0] != '+':
                            logger.debug(
                                f"Wrongly formated phone_number column in {key}")
                # ----------------------------------------------------------------------------------------------------------------------------------------------
                # cleaning the name
                self.talent_csv_df_dict[key][[
                    'name']] = self.talent_csv_df_dict[key]['name'].str.title()
                self.talent_csv_df_dict[key]['name'] = self.talent_csv_df_dict[key]['name'].str.replace(
                    ' - ', '-', regex=True)
                self.talent_csv_df_dict[key]['name'] = self.talent_csv_df_dict[key]['name'].str.replace(
                    "' ", '', regex=True)
                ...
                self.talent_csv_df_dict[key][[
                    'invited_by']] = self.talent_csv_df_dict[key]['invited_by'].str.title()
                self.talent_csv_df_dict[key]['invited_by'] = self.talent_csv_df_dict[key]['invited_by'].str.replace(
                    'Bruno Belbrook', 'Bruno Bellbrook', regex=True)
                self.talent_csv_df_dict[key]['invited_by'] = self.talent_csv_df_dict[key]['invited_by'].str.replace(
                    'Fifi Etton', 'Fifi Eton', regex=True)

                # ----------------------------------------------------------------------------------------------------------------------------------------------
                # making gender title case to keep them constant
                self.talent_csv_df_dict[key][[
                    'gender']] = self.talent_csv_df_dict[key]['gender'].str.title()

                # making citytitle case to keep them constant
                self.talent_csv_df_dict[key][[
                    'city']] = self.talent_csv_df_dict[key]['city'].str.title()
                # ----------------------------------------------------------------------------------------------------------------------------------------------
                # cleaning the university

                self.talent_csv_df_dict[key]['uni'].replace(
                    {'-': ','}, inplace=True)
                self.talent_csv_df_dict[key]['uni'].replace(
                ...
                self.talent_csv_df_dict[key]['uni'].replace(
                    {"'S": "'s"}, regex=True, inplace=True)
                self.talent_csv_df_dict[key]['uni'].replace(
                    {"St\.": "St "}, regex=True, inplace=True)
                self.talent_csv_df_dict[key]['uni'].replace(
                    {"Saint George's": "St George's"}, regex=True, inplace=True)

                # cleaning the email
                self.talent_csv_df_dict[key]['email'] = self.talent_csv_df_dict[key]['email'].str.replace(
                    ';', '', regex=True)
                # ----------------------------------------------------------------------------------------------------------------------------------------------
                # cleaning the invited_date
                self.talent_csv_df_dict[key]['invited_date'] = self.talent_csv_df_dict[key]['invited_date'].astype(
                    str).str.replace('.0', '', regex=True)
                self.talent_csv_df_dict[key]['month_short'] = self.talent_csv_df_dict[key]['month'].str.slice(
                    0, 3)
                self.talent_csv_df_dict[key]['invite_year'] = self.talent_csv_df_dict[key]['month'].str.extract(
                    r'\b(\w+)$', expand=True)
                ...
                self.talent_csv_df_dict[key]['invite_date'] = self.talent_csv_df_dict[key]['invite_date'].astype(
                    'object')

                # ----------------------------------------------------------------------------------------------------------------------------------------------
                # cleaning the date of birth column
                self.talent_csv_df_dict[key]['dob'] = pd.to_datetime(
                    self.talent_csv_df_dict[key]['dob'], format='%d/%m/%Y').dt.date

                # dropping unwanted columns to keep only the data we want
                self.talent_csv_df_dict[key].drop(columns=[
                                                  'id', 'invited_date', 'month_short', 'invite_year', 'month'], axis=1, inplace=True)
                ...
                if self.talent_csv_df_dict[key][self.talent_csv_df_dict[key].duplicated(subset=['candidate_name', 'dob', 'address', 'date'])].empty == False:
                    logger.warning(
                        f" Duplicated rows: {self.talent_csv_df_dict[key][self.talent_csv_df_dict[key].duplicated(subset=['candidate_name','dob','address','date'])]}")

                # logging any dataframes with incorectly named columns
                erd_cols = ['candidate_name', 'gender', 'dob', 'email', 'city', 'address',
                            'postcode', 'phone_number', 'uni_name', 'degree_result', 'staff_name', 'date']
                if erd_cols.sort() != (list(self.talent_csv_df_dict[key].columns)).sort():
                    logger.debug(
                        f'{key} df has incorrect columns: {list(self.talent_csv_df_dict[key].columns)}')
                ...
                if self.talent_csv_df_dict[key][self.talent_csv_df_dict[key][cols].isna().all(1)].empty == False:
                    logger.warning(
                        f'Rows with no condidate contact info {self.talent_csv_df_dict[key][self.talent_csv_df_dict[key][cols].isna().all(1)]}')

                # self.combined_applicants_df = self.combined_applicants_df.append(self.talent_csv_df_dict[key])
                self.combined_applicants_df = pd.concat(
                    [self.combined_applicants_df, self.talent_csv_df_dict[key]], ignore_index=True)
                self.combined_applicants_df = self.combined_applicants_df.replace({
                                                                                  pd.NaT: None})
                ...
                for name in self.talent_csv_df_dict[key]['staff_name'].unique():
                    match = get_matches(
                        str(name), self.talent_csv_df_dict[key]['staff_name'])
                    if match != None:
                        logger.warning(
                            f'{name} is {match[1]}% similar to {match[0]} ')
