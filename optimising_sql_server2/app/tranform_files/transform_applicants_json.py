...
class transformJsonFiles():
    ...
    def __init__(self):
        self.json_objects = getFiles(
            'data21-final-project-preassignment', 'Talent', '.json')
    ...
    def make_dataframes(self):
        ...
        if self.json_files_dict != None:
            for json_id in tqdm(self.json_files_dict, unit='json_files', desc='Transforming Json_files', position=0, total=len(self.json_files_dict)):
                ...
                self.sparta_day_df2 = self.sparta_day_df2.append(
                    df_inserts, ignore_index=True)
                self.sparta_day_df2['date'] = self.sparta_day_df2['date'].astype(
                    'datetime64')
                self.sparta_day_df2['date'] = self.sparta_day_df2['date'].astype(
                    str)
                ...
                for weakness in applicant['weaknesses']:
                    ...
                    self.weaknesses_junc_df = self.weaknesses_junc_df.append(
                        df_inserts, ignore_index=True)
                ...
                for strength in applicant['strengths']:
                    ...
                    self.strengths_junc_df = self.strengths_junc_df.append(
                        df_inserts, ignore_index=True)
                ...
                if 'tech_self_score' in applicant.keys():
                    for tech in applicant['tech_self_score']:
                        ...
                        self.tech_junc_df = self.tech_junc_df.append(
                            df_inserts, ignore_index=True)
                else:
                    df_inserts = {
